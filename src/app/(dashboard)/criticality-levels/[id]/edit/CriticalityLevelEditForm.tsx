...
export default function CriticalityLevelEditForm({ initial }: { initial?: CriticalityLevel }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = initial
        ? await fetch(`/api/criticality-levels/${initial.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          })
        : await fetch("/api/criticality-levels", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          });
      ...
      router.push("/criticality-levels");
      router.refresh();
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="form-card">
        ...
        <div className="form-group">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <input className="form-input" value={form.color_hex} onChange={e => set("color_hex", e.target.value)}
              placeholder="#D32F2F" style={{ flex: 1 }} />
          </div>
        </div>
        <div className="form-group">
      ...
      <div className="form-row">
        <div className="form-group">
          <label className="form-label">Priorità (valore numerico)</label>
