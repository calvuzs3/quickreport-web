...
export default function IslandTypeEditForm({ type }: { type: IslandType }) {
  ...
  const [form, setForm] = useState({
    ...
    maintenance_interval_days: type.maintenance_interval_days,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/island-types/${type.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...type, ...form, updated_at: Date.now() }),
      });
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <Link href="/island-types" style={{ color: "var(--color-text-muted)", fontSize: 13 }}>
            ← Tipi isola
          </Link>
          <h1 className="page-title" style={{ marginTop: 4 }}>Modifica tipo isola</h1>
        </div>
      </div>
